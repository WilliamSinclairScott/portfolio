              ...
              <Dialog.Trigger asChild>
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-black p-6 rounded-lg border border-white/10 w-full max-w-md">
                  ...
                  <label className="block">
                    ...
                    {upload ? (
                    <button
                      className="mt-4 w-full bg-gradient-to-r from-green-600 to-green-800 hover:from-green-700 hover:to-green-900 text-white font-semibold py-2 rounded-md transition-colors flex items-center justify-center"
                      onClick={async () => {
                      if (upload) {
                      setLoading(true);
                      setError(null);
                      await handleUpload(year, upload);
                      await new Promise((resolve) => setTimeout(resolve, 3000));
                      const result = await fetchFiles(year);
                      setFiles(result);
                      setUpload(null);
                      setLoading(false);
                      }
                    ...
                    >
                      {loading ? (
                      <div className="animate-spin h-5 w-5 border-4 border-white border-t-transparent rounded-full mr-2" />
                      ) : null}
                      UPLOAD
                    </button>
                    ) : null}
